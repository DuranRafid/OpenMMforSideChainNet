...
class OpenMMBatchFunction(Function):
    ...
    def forward(ctx, coords,input):
        ...
        for protein_index in range(input.shape[0]):
            seq = ''.join(
                [VOCAB.int2char(seq_int.index(1)) for seq_int in input[protein_index, :, :21].tolist()]).replace('_','')
        ...
        ctx.coords = coords
        ret_val = torch.as_tensor([pdb_mm.get_potential_energy()._value for pdb_mm in pdb_mmlist])
        return ret_val
    ...
    def backward(ctx, grad_output=None):
        ...
        force_arr = np.zeros((coords.shape[0], coords.shape[1], coords.shape[2]), dtype=np.float64)
        for batch_index in range(coords.shape[0]):
            ...
            assert len(forces) == coord_sum[coord_sum != 0].shape[0], "Not generating force for all existing atoms"
...
def errorcheck(data, data1):
    ...
    assert data_non_zeros.tolist() == data1_non_zeros.tolist(), "Gradient for non existent atom"
...
if __name__ == '__main__':
    ...
    dataloaders = scn.load(casp_version=12, with_pytorch="dataloaders", batch_size=2, num_workers=0,use_dynamic_batch_size=False)
